...
import { loginLivreur } from '@/lib/livreur-auth';
import type { LivreurSession } from '@/lib/livreur-auth';

export function LivreurLogin({ onSuccess }: { onSuccess: (s: LivreurSession) => void }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const res = await loginLivreur(phone, password);
    setLoading(false);
    if (!res.ok) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-900 via-emerald-800 to-emerald-950 flex items-center justify-center p-4">
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-2xl p-6">
        <div className="text-center mb-6">
          ...
          <h1 className="text-2xl font-bold text-emerald-900">App Livreur KOUKA</h1>
          <p className="text-sm text-gray-500 mt-1">
            {mode === 'login' ? 'Connecte-toi pour voir tes livraisons' : 'Crée ton mot de passe (1ʳᵉ connexion)'}
          </p>
        </div>
        ...
        <form onSubmit={submit} className="space-y-3">
          <div>
            ...
            <input
              ...
              className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2.5 text-base focus:border-emerald-600 focus:ring-2 focus:ring-emerald-200 outline-none"
            />
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-3 mt-2 disabled:opacity-50"
          >
